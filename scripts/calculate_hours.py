...
def get_issue_id(repo_owner, repo_name, github_token):
    ...
    # Debugging: Check token length before making API call
    print(f"🔍 Debug: GitHub Token Length: {len(github_token)}")

    response = requests.get(url, headers=headers)

    if response.status_code == 401:
        print("❌ ERROR: GitHub API Authentication Failed! Check GH_TOKEN.")
        print(f"Response: {response.text}")
        return None
    ...
    try:
        ...
        if not issue_data or not isinstance(issue_data, list) or "number" not in issue_data[0]:
